...
/**
 * The current business's logo (falls back to the bundled logo), cropped to a circle with a green ring around it.
 *
...
export function BrandLogo({
  ...
  ringWidth?: number;
  /** Override the logo image; defaults to the signed-in business's logo. */
  src?: string;
}) {
  const { business } = useSession();
  const logoSrc = src ?? business?.logo_url ?? logo;
  return (
    <span
      className={cn("brand-logo", animated && "brand-logo-animated", className)}
      style={{ "--brand-ring-w": `${ringWidth}px` } as CSSProperties}
    >
      <img src={logoSrc} alt={alt} draggable={false} className="brand-logo-img" />
    </span>
  );
}