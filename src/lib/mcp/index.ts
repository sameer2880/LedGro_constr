...
export default defineMcp({
  ...
  version: "0.1.0",
  instructions:
    "Tools for the centring-rental app. NOTE: these tools call the database as an anonymous visitor, which no longer has any access once the multi-business lockdown is applied — they return nothing until they are re-implemented with caller authentication. Use `list_rentals` and `business_summary` to read rental records, overdue items and revenue; `create_rental` and `update_rental` to record or close out rentals; `list_diary_notes` and `create_diary_note` for daily diary entries (labour, expense, payment, reminder). Dates are YYYY-MM-DD and amounts are in INR.",
